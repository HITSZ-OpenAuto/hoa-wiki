...
确实，博客并没有属于自己的仓库。课程信息页面右侧的 `在 GitHub 上编辑` 各课程仓库入口是我们做的特殊处理。
...
blog 下的文件夹名是 `auto-survival-guide`，这是博客名称标识符。
...
`embedded math overview ros` 文件夹下各有一个 `index.zh-cn.md`，这就是正文。
...
![](./img/next-prev.png)
_<center>左边是 next，右边是 prev。</center>_
